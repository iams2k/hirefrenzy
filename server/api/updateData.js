  ...
  if (member == "student") {
    ...
    try {
      const studentUpdate = await Student.updateOne(
        { userName },
        { $set: { name, email, phone, address, department, portfolio, links } }
      );
      if (studentUpdate) {
    ...
    try {
      const placementUpdate = await Placement.updateOne(
        { userName },
        { $set: { name, email, phone, address, department } }
      );
      if (placementUpdate) {
        return res.status(202).json({ message: "Update Successful" });
      } else {
    ...
    try {
      const placementUpdate = await Admin.updateOne(
        { userName },
    ...
    try {
      const companyUpdate = await Company.updateOne(
        { userName },
        { $set: { name, email, phone, address, companyLink } }
      );
      if (companyUpdate) {
        return res.status(202).json({ message: "Update Successful" });
      } else {
