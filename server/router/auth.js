  ...
  if (member == "Company") {
    ...
    try {
      ...
      if (companyExist) {
        return res
          .status(422)
          .json({ message: "Already Company with this Username registered" });
      }
    ...
    try {
      ...
      if (studentExist) {
        return res
          .status(422)
          .json({ message: "Already Student with this Username registered" });
      }
      if (password !== rpassword) {
        return res.status(422).json({ message: "Both Password must be same" });
      }
    ...
    try {
      ...
      if (placementExist) {
        return res.status(422).json({
          message:
            "Already Placement Coordinator with this Username registered",
        });
      }
      if (password !== rpassword) {
        return res.status(422).json({ message: "Both Password must be same" });
      ...
      res
        .status(201)
        .json({ message: "Placement Coordinator registered successfully" });
    } catch (err) {
      console.log(err);
    }
    ...
    try {
      ...
      if (adminExist) {
        return res
          .status(422)
          .json({ message: "Already Admin with this Username registered" });
      }
      if (password !== rpassword) {
        return res.status(422).json({ message: "Both Password must be same" });
      }
      const admin = new Admin({ userName, password, name, email });
...
router.post("/register", async (req, res) => {
  ...
  if (role == "company") {
    ...
    const { userName, name, password, email, phone, department, address } =
      req.body;
      ...
      if (studentLogin) {
        if (!studentLogin.accept) {
          return res.status(402).json({
            message: "Once a Placement Coordinator or Admin will Verify You :)",
          });
        }
    ...
    try {
      ...
      if (placementLogin) {
        if (!placementLogin.accept) {
          return res
            .status(402)
            .json({ message: "Once a Admin will Verify You :)" });
        }
        const validPassword = await bcrypt.compare(
          password,
          placementLogin.password
        );
        if (validPassword) {
          //tokenization
